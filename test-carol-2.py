...
for root_dir in args.run_folders:
    print(root_dir)
    assert os.path.exists(os.path.join(root_dir, "surface"))
    subfolders = [ f.name for f in os.scandir(root_dir) if f.is_dir() if f.name != "surface"]

    try:
        data_objects = read_trajectory_extract_features(a2g, os.path.join(root_dir, "surface", "OUTCAR"))
    except:
        import pdb;pdb.set_trace()
    surface_energy = data_objects[1].y_relaxed
    for runname in subfolders:
            # Extract Data object
        filename = os.path.join(root_dir, runname, "OUTCAR")
